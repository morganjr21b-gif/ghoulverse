  ...
  return (
    <main style={{ maxWidth: 700, margin: "0 auto", padding: "24px 24px 90px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24 }}>
        {prevChapter ? (
          <a href={`/read/${prevChapter.id}`} style={navBtn}>← Ch. {prevChapter.chapter_number}</a>
        ) : <span />}
        {nextChapter ? (
          <a href={`/read/${nextChapter.id}`} style={navBtn}>Ch. {nextChapter.chapter_number} →</a>
        ) : <span />}
      </div>
      ...
      <div style={toolbar}>
        <a href={`/series/${chapter.series_id}`} style={toolbarItem}>
          <ListIcon /><span style={toolbarLabel}>List</span>
        </a>
        <div style={toolbarItem}>
          <CommentIcon /><span style={toolbarLabel}>{comments.length}</span>
        </div>
        <button
          onClick={() => setSaved(!saved)}
          style={{ ...toolbarItem, background: "none", border: "none", cursor: "pointer" }}
        >
          <BookmarkIcon filled={saved} /><span style={toolbarLabel}>Save</span>
        </button>
        <a href="/settings" style={toolbarItem}>
          <SettingsIcon /><span style={toolbarLabel}>Settings</span>
        </a>
      </div>
    </main>
  );
...
function ListIcon() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" /></svg>;
}
function CommentIcon() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z" /></svg>;
}
function BookmarkIcon({ filled }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill={filled ? "#E63946" : "none"} stroke="currentColor" strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z" /></svg>;
}
